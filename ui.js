...
function openAdd(){
  const d=new Date(),f=document.getElementById('measurementForm');
  f.reset();
  f.elements.date.value=d.toISOString().slice(0,10);
  f.elements.time.value=d.toTimeString().slice(0,5);
  mDialog.showModal();
}

document.getElementById('addMeasurementBtn').addEventListener('click',openAdd);
...
document.getElementById('measurementForm').addEventListener('submit',e=>{
  e.preventDefault();
  const fd=new FormData(e.target),obj={id:`${fd.get('date')}-${Date.now()}`,date:fd.get('date'),time:fd.get('time')||null};
...
const labDialog=document.getElementById('labDialog');
document.getElementById('addLabBtn').addEventListener('click',()=>{
  ...
  f.elements.unit.value='mg/dL';
  labDialog.showModal();
});
document.getElementById('labForm').addEventListener('submit',e=>{
  e.preventDefault();
  const fd=new FormData(e.target);
  labs.push({id:`lab-${Date.now()}`,date:fd.get('date'),key:fd.get('key'),value:Number(fd.get('value')),unit:fd.get('unit')||LAB_DEFS[fd.get('key')].unit});
...
document.getElementById('reportForm').addEventListener('submit',e=>{
  e.preventDefault();
  const fd=new FormData(e.target);
  const lines=firstNonEmptyLine(fd.get('bullets'));
  reports.push({
    id:`report-${Date.now()}`,
    date:fd.get('date')||'',
...
document.getElementById('exportBtn').addEventListener('click',()=>{
  if(!measurements.length&&!labs.length&&!reports.length){alert('Nenhum dado para exportar.');return}
...
document.getElementById('importInput').addEventListener('change',async e=>{
  ...
  try{
    ...
    if(!validateMeasurements(payload.measurements))throw new Error('Bioimpedâncias inválidas');
    if(!validateLabs(payload.labs))throw new Error('Exames inválidos');
    if(!validateReports(payload.reports))throw new Error('Laudos inválidos');
    profile=payload.profile||{};
    measurements=normalizeMeasurements(payload.measurements);
    labs=normalizeLabs(payload.labs);
...
document.getElementById('resetBtn').addEventListener('click',()=>{
  if(confirm('Apagar todos os dados salvos neste aparelho?')){
    measurements=[];labs=[];reports=[];profile={};
